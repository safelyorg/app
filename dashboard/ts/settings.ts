...
async function loadSettingsData(): Promise<void> {
  ...
  try {
    ...
    (document.getElementById("settings-name") as HTMLElement).textContent =
...
async function handleGoogleButtonClick(): Promise<void> {
  ...
  if (btn.dataset.connected === "true") {
    ...
    try {
      ...
      if (signinMethodEl) signinMethodEl.textContent = t("dash.settings.email_magic_link", "Email magic link");
      const statusEl = document.getElementById("google-status-message");
      if (statusEl) {
        statusEl.textContent = t(
          "dash.settings.google_disconnected",
...
function checkGoogleConnectResult(): void {
  ...
  if (error === "google_already_linked") {
    ...
    alert(
      t(
        "dash.settings.google_email_mismatch",
        "That Google account uses a different email address than your Safely account. Please connect a Google account that uses the same email address.",
      ),
    );
