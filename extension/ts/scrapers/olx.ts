...
(function () {
   ...
   */
  function scrapeOLX(): OlxScrapedData {
    const data: OlxScrapedData = {
      listing_id: null,
      title: null,
      price: null,
      description: null,
    ...
    const postedByLabel = Array.from(
      document.querySelectorAll<HTMLElement>("span._9083bec6"),
    ).find((el) => el.innerText.trim() === "Posted by");
    if (postedByLabel && postedByLabel.nextElementSibling) {
      const nameEl = postedByLabel.nextElementSibling.querySelector("span") as HTMLElement | null;
    ...
    const profileLink = document.querySelector("a.da952dfc") as HTMLAnchorElement | null;
    if (profileLink) {
    ...
    if (locationSvg && locationSvg.parentElement) {
      data.seller_location = (locationSvg.parentElement as HTMLElement).innerText.trim() || null;
    } else {
      const pinSvg = Array.from(document.querySelectorAll("svg")).find(
        (svg) => svg.getAttribute("viewBox") === "0 0 15 15",
      );
      data.seller_location =
        pinSvg && pinSvg.parentElement
          ? (pinSvg.parentElement as HTMLElement).innerText.trim() || null
          : null;
    }

    const lastActiveEl = document.querySelector(
  ...
  (window as any).__safelyScrapers.scrapeOLX = scrapeOLX;
})();
